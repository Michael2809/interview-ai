    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-soft">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-gray-mid hover:bg-gray-100 transition-colors">
            <X size={18} />
          </button>
        </div>
  ...
  const content = (
    <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-lavender/30 transition-colors">
      ...
      <div className="flex items-center gap-2 shrink-0 ml-2">
        {score != null && <span className={`text-xs font-medium px-2 py-1 rounded-full ${scoreColor(score)}`}>{score}/10</span>}
        {stageId && <ChevronRight size={14} className="text-gray-mid" />}
      </div>
    </div>
  )
  if (stageId) {
    return <Link href={`/interview/${stageId}/transcript`} className="block hover:opacity-90 transition-opacity">{content}</Link>
  }
  return content
}
...
export default function DashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const completedNames = new Set(transcripts.map((r) => r.candidate_name?.toLowerCase()).filter(Boolean))
    const ongoingArr = invitedArr.filter((c) => !completedNames.has(c.email?.toLowerCase()))
  ...
  const panelContent = {
    ...
    invited: {
      ...
      content: invitedList.length === 0
        ...
        : invitedList.map((c, i) => (
          <CandidateRow key={i} email={c.email} roleTitle={c.roleTitle}
            date={c.invited_at ? new Date(c.invited_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : null} />
        )),
    ...
    completed: {
      ...
      content: completedList.length === 0
        ...
        : completedList.map((c, i) => (
          <CandidateRow key={i} name={c.name} roleTitle={c.roleTitle} score={c.score} stageId={c.stageId} />
        )),
    ...
    score: {
      ...
      content: completedList.filter((c) => c.score != null).length === 0
        ...
        : completedList.filter((c) => c.score != null).map((c, i) => (
          <CandidateRow key={i} name={c.name} roleTitle={c.roleTitle} score={c.score} stageId={c.stageId} />
        )),
