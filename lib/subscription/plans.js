...
/**
 * Look up a plan by its Dodo Payments product ID — the reverse
 * direction of the mapping `dodo_product_id` provides. Used by the
 * webhook to turn an incoming `product_id` into one of our plan keys,
 * and by the change-plan route in reverse (plan key -> product id).
 * Returns null if no plan is mapped to that product id.
 */
export async function getPlanByDodoProductId(supabase, productId) {
  if (!productId) return null
  const plans = await getPlans(supabase)
  return plans.find((p) => p.dodo_product_id === productId) || null
}
