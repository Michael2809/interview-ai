...
/* There is no trial. A new workspace starts on the Free plan, which is
   permanent and has a candidate limit of zero — you can set a role up
   and look around, but nobody interviews anybody until a plan is paid
   for. The far-future period end exists because effectiveStatus() reads
   it; a Free plan is not something that lapses. */
const FREE_PERIOD_YEARS = 100
...
export async function ensureSubscription(supabase, userId) {
  ...
  farFuture.setFullYear(farFuture.getFullYear() + FREE_PERIOD_YEARS)

  const { data: sub, error: subErr } = await supabase
    .from('subscriptions')
    .insert({
      user_id: userId,
      plan_key: PLAN_KEYS.TRIAL,           // the Free plan; key kept for compatibility
      status: SUBSCRIPTION_STATES.ACTIVE,  // never expires — it was never a trial
...
export async function claimPendingDodoLink(supabase, userId) {
  ...
  const { data: pending } = await supabase
    ...
    .select('*')
    /* eq, not ilike. `_` and `%` are wildcards in a LIKE pattern, so
       john_doe@acme.com also matched johnXdoe@acme.com; maybeSingle()
       then errored on multiple rows and a real payment was parked as
       unclaimed instead of activating. Addresses are stored lowercased
       by the signup trigger. */
...
export function effectiveStatus(sub) {
  ...
  /* Cancelled was never time-checked, so it stayed cancelled forever —
     and no gate in the product blocks a cancelled subscription. Cancel
     meant free indefinitely. A cancellation keeps what was paid for
     until the period ends, then expires like anything else. */
  if (sub.status === SUBSCRIPTION_STATES.CANCELLED && end <= now) return SUBSCRIPTION_STATES.EXPIRED
  return sub.status
}
