...
export const ALLOWED_TRANSITIONS = Object.freeze({
  ...
  /* `expired` added. A cancelled subscription that reaches the end of
     its paid period has expired — that is the normal end of the
     lifecycle, not an illegal move. Rejecting it made the provider's
     `subscription.expired` event throw, and (with the webhook bug it
     travelled with) the customer stayed `cancelled` indefinitely, which
     no gate in the product blocks. */
